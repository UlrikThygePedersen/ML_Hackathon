...
def prepare(df: pd.DataFrame) -> pd.DataFrame:
    ...
    out["value_retention"] = out["market_value_in_eur"] / out["highest_market_value_in_eur"]
...
fig = px.bar(
    missing, x="pct_missing", y="column", orientation="h",
...
fig = px.bar(
    ...
    color="position", labels={"count": "Players"},
    height=300,
)
...
with col2:
    ...
    scatter_df = df[["market_value_in_eur", "highest_market_value_in_eur", "position"]].dropna()
    fig = px.scatter(
        scatter_df,
    ...
    max_val = scatter_df[["market_value_in_eur", "highest_market_value_in_eur"]].max().max()
    fig.add_trace(go.Scatter(
        x=[1e4, max_val], y=[1e4, max_val],
...
fig = px.scatter(
    age_df, x="age", y="market_value_in_eur",
    color="position", log_y=True, opacity=0.35,
    trendline="lowess",
...
with col1:
    ...
    fig = px.box(
        box_df, x="position", y="market_value_in_eur",
        color="position", log_y=True,
...
with col2:
    ...
    fig = px.bar(
        foot_df, x="foot", y="market_value_in_eur",
        color="foot",
...
with col1:
    ...
    fig = px.scatter(
        height_df, x="height_in_cm", y="market_value_in_eur",
        color="position", log_y=True, opacity=0.4,
        trendline="lowess",
        labels={
...
with col2:
    ...
    fig = px.scatter(
        caps_df, x="international_caps", y="market_value_in_eur",
        color="position", log_y=True, opacity=0.5,
        trendline="lowess",
        labels={
