...
def apply_strategy(df: pd.DataFrame, col: str, strategy: str) -> pd.DataFrame:
    ...
    if strategy == "Mean":
        ...
        df[col] = (
            df[col]
            .fillna(df.groupby("position")[col].transform("median"))
...
if not cols_with_nulls:
    ...
    for col in cols_with_nulls:
        ...
        default = DEFAULT_STRATEGIES.get(
            col, "Median" if raw[col].dtype != "object" else "Mode"
        )
...
if df_clean is not None:
    ...
    col2.metric(
        "Rows after", f"{len(df_clean):,}", delta=f"{len(df_clean) - len(raw):,}"
    )
    ...
    after = (
        df_clean.reindex(columns=selected_cols).isnull().mean().mul(100).rename("after")
    )
    compare = (
